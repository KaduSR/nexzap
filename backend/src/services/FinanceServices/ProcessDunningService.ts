...
export const ProcessDunningService = async (): Promise<void> => {
  ...
  const rules = await (DunningSettings as any).findAll({ where: { isActive: true } });
  ...
  for (const rule of rules) {
      ...
      logger.info(`[Dunning] Processing Rule: ${rule.frequencyType} ${rule.days} days. Target Date: ${dateStr}`);
      ...
      for (const invoice of invoices) {
          try {
              ...
              let contact = await (Contact as any).findOne({ where: { number: { [Op.like]: `%${phone}%` } } });
              
              if (!contact) {
                  // Optional: Create contact if not exists (Implementation dependent)
                  // For now, we skip to avoid spamming wrong numbers
                  logger.warn(`[Dunning] Contact not found for phone ${phone}. Skipping.`);
                  continue; 
              }

              // Get default Whatsapp Connection
              const whatsapp = await (Whatsapp as any).findOne({ where: { status: "CONNECTED" } });
              if (!whatsapp) {
              ...
              logger.info(`[Dunning] Message sent to ${contact.name} (${invoice.id})`);

          } catch (err) {
              logger.error(`[Dunning] Error processing invoice ${invoice.id}: ${err}`);
          }
      }
  }
