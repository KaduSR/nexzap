...
const UnifiedAiService = async ({ companyId, prompt, history, image }: Request): Promise<string | null> => {
  try {
    const settings = await (Setting as any).findAll({
        where: {
            key: ['ai_provider', 'ai_api_key', 'ai_enabled', 'ai_system_prompt', 'ai_model']
        }
    });
    ...
    if (!isEnabled) {
      logger.info("Unified AI Service is disabled globally. Skipping response.");
      return null;
    }

    // 2. Determine provider and credentials.
    const provider = getSettingValue("ai_provider", "gemini");
    const systemPrompt = getSettingValue("ai_system_prompt", "Você é um assistente virtual útil.");
    ...
    if (!apiKey) {
        logger.warn(`API Key for provider '${provider}' is not configured. AI will not respond.`);
        return null;
    }
    ...
    if (image) {
        if (provider === 'gemini') {
            ...
            const imgModel = model && model.includes('vision') ? model : 'gemini-3-flash-preview';
    ...
    if (error instanceof Error && (error.message.includes('API key') || error.message.includes('authentication'))) {
        logger.error("Authentication error with AI provider. Check if the API Key is valid.");
    }
    return null; 
  }
};
