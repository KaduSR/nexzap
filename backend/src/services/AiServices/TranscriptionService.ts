...
export const TranscriptionService = async ({ companyId, audio }: Request): Promise<string> => {
    try {
        const settings = await (Setting as any).findAll({
            where: {
                key: ['ai_transcription_provider', 'ai_transcription_api_key', 'ai_transcription_model']
            }
        });

        const getSettingValue = (key: string, defaultValue: string | null = null) => {
        ...
        const model = getSettingValue("ai_transcription_model", "gemini-2.5-flash-native-audio-preview-09-2025");
        ...
        if (provider === 'gemini') {
            ...
            throw new AppError(`Transcription provider '${provider}' is not supported yet.`, 501);
        }
        ...
        throw new AppError("Failed to communicate with AI transcription service.", 500);
    }
};