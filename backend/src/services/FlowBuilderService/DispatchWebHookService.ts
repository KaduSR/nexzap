...
const DispatchWebHookService = async ({ ticket, node, contact }: Params) => {
    logger.info(`[FlowBuilder] Dispatching Webhook/Action for Ticket ${ticket.id}`);

    // Verifica se é um nó de integração ISP
    if (["ixc_action", "integration_finance", "ixc_smart_finance", "ixc_diagnostic"].includes(node.type)) {
        const wbot = getWbot(ticket.whatsappId);
        const remoteJid = `${contact.number}@${contact.isGroup ? "g.us" : "s.whatsapp.net"}`;
        ...
        if (node.type === "ixc_smart_finance" || node.type === "ixc_diagnostic") {
             await wbot.sendMessage(remoteJid, { text: "🔍 Identificando seu cadastro..." });
             ...
             if (client) {
                 await wbot.sendMessage(remoteJid, { text: `Olá *${client.razao.split(' ')[0]}*! Encontrei seu cadastro.` });
        ...
        if (!client) {
            await wbot.sendMessage(remoteJid, { text: "⚠️ Não consegui localizar seu cadastro automaticamente. Por favor, digite seu CPF/CNPJ na próxima etapa." });
            return;
        }
        ...
        if (node.type === "ixc_smart_finance" || (node.type === "ixc_action" && node.data?.resource === "fn_areceber")) {
            
            // Checa status de bloqueio
            if (client.bloqueado === "S") {
                await wbot.sendMessage(remoteJid, { text: "🔒 Notei que sua conexão consta como *BLOQUEADA* por pendência financeira." });
                ...
                if (unlock.success) {
                    ...
                    await wbot.sendMessage(remoteJid, { text: "💡 Dica: O desbloqueio de confiança só pode ser usado uma vez por fatura." });
                }
            }
            ...
            if (invoices.length > 0) {
                ...
                const pixCode = invoices.find(i => i.pix_copia_cola && i.pix_copia_cola.length > 10)?.pix_copia_cola;
                if (pixCode) {
                    await wbot.sendMessage(remoteJid, { text: pixCode });
                }

            } else {
                await wbot.sendMessage(remoteJid, { text: "✅ Parabéns! Não constam faturas em aberto. Sua conexão está em dia." });
            }
        } 
        
        // Ação: AUTO DIAGNÓSTICO (Técnico)
        else if (node.type === "ixc_diagnostic" || (node.type === "ixc_action" && node.data?.resource === "diagnostico")) {
            await wbot.sendMessage(remoteJid, { text: "🛠️ Realizando teste de conexão no seu equipamento, aguarde um momento..." });
        ...
        else if (node.type === "ixc_action" && node.data?.resource === "create_ticket") {
            const protocolo = Math.floor(Math.random() * 9000000) + 1000000;
            await wbot.sendMessage(remoteJid, { text: `📝 Protocolo criado: *${protocolo}*\n\nUm técnico analisará seu caso em breve.` });
        }
