...
interface Node {
    id: string;
    type: 'message' | 'choice' | 'input' | 'transfer' | 'tag' | 'condition' | 'wait' | 'trigger';
    data: any;
}
...
export const FlowEngineService = async (
    ...
    if (!ticket.flowCampaignId || ticket.flowStopped) {
        ...
        if (flow) {
            logger.info(`[FlowEngine] Starting flow "${flow.name}" for Ticket ${ticket.id}`);
            ...
            await processNode(ticket, flow.flow.nodes[0], flow.flow.nodes, wbot, msgBody);
    ...
    const flowCampaign = await (FlowCampaign as any).findByPk(ticket.flowCampaignId);
    if (!flowCampaign) {
    ...
    if (currentNode.type === 'choice') {
        ...
        if (selectedOption) {
            ...
            await wbot.sendMessage(remoteJid, { text: "Opção inválida. Por favor, tente novamente." });
...
const processNode = async (ticket: Ticket, node: Node, nodeList: Node[], wbot: any, lastInput: string) => {
    const remoteJid = `${ticket.contact.number}@${ticket.isGroup ? "g.us" : "s.whatsapp.net"}`;
    ...
    if (node.type === 'message' || node.type === 'trigger' || node.type === 'input') {
        const text = replaceVariables(node.data.content || "", ticket.flowContext, ticket.contact);
        await wbot.sendMessage(remoteJid, { text });

        // If it's just a message (not an input question), move automatically
        if ((node.type === 'message' || node.type === 'trigger') && node.data.nextId) {
    ...
    // NODE: TRANSFER (HANDOVER)
        ...
        await wbot.sendMessage(remoteJid, { text: "Aguarde um momento, estamos transferindo para um atendente..." });
    }
    ...
    // NODE: CONDITION
        ...
        if (operator === 'contains') conditionMet = String(storedValue).includes(value);
