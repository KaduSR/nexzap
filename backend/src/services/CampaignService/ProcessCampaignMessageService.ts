...
const ProcessCampaignMessageService = async ({
  ...
  if (!campaignShipping.contact) {
    await (campaignShipping as any).update({ deliveredAt: new Date(), confirmationRequested: true }); // Marca como erro/ignorado
    return;
  }
  ...
  const remoteJid = `${contactNumber}@${contactNumber.length > 13 ? "g.us" : "s.whatsapp.net"}`;
  ...
  try {
    ...
    const messageBody = body.replace(/{{\s*name\s*}}/gi, campaignShipping.contact.name || "Cliente");
    ...
    logger.info(`Campaign ${campaign.name}: Waiting delay for ${contactNumber}...`);
