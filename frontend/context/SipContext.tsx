...
interface SipContextData {
  ...
  connectionStatus: "Connected" | "Disconnected" | "Connecting" | "Error" | "Registered";
...
export const SipProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [recentCalls, setRecentCalls] = useState<CallLog[]>([
      { id: '1', number: '5511999887766', type: 'incoming', date: new Date(Date.now() - 1000 * 60 * 30), duration: 120, name: 'João Silva' },
      { id: '2', number: '5511988776655', type: 'missed', date: new Date(Date.now() - 1000 * 60 * 60 * 2), name: 'Maria Souza' },
      { id: '3', number: '5511977665544', type: 'outgoing', date: new Date(Date.now() - 1000 * 60 * 60 * 24), duration: 45, name: 'Suporte Técnico' },
  ]);
  ...
  const makeCall = async (number: string) => {
    if (connectionStatus !== "Registered" && connectionStatus !== "Connected") {
        alert("Erro: Softphone não registrado. Verifique as credenciais em Integrações.");
        return;
    }
