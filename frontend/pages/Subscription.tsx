    ...
    <div className="p-8 space-y-10 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      {status === 'success' && (
          <div className="bg-emerald-500/10 border border-emerald-500/30 p-6 rounded-[24px] text-center mb-8 animate-in zoom-in duration-300">
              ...
              <h2 className="text-2xl font-black text-emerald-500 mb-2">Pagamento Confirmado!</h2>
              <p className="text-slate-400">Sua assinatura está ativa. Aproveite todos os recursos.</p>
              <button onClick={() => navigate('/dashboard')} className="mt-6 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold transition-all">
                  Ir para o Dashboard
              </button>
          </div>
      )}
      ...
      {status === 'canceled' && (
          <div className="bg-amber-500/10 border border-amber-500/30 p-4 rounded-2xl flex items-center justify-center gap-3 text-amber-500 font-bold mb-8">
              <AlertTriangle /> O pagamento foi cancelado. Nenhuma cobrança foi realizada.
          </div>
      )}
      ...
      <div className="flex justify-center mt-8">
          <div className="bg-slate-900 p-1.5 rounded-2xl border border-slate-800 flex flex-col sm:flex-row gap-1 shadow-xl">
              ...
              >
                  <CreditCard size={18} />
                  Cartão de Crédito
                  <span className="hidden sm:inline opacity-60 text-[10px] uppercase tracking-wider ml-1 bg-black/20 px-1.5 py-0.5 rounded">Auto</span>
              </button>
              <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-4">
          {loading ? (
              <div className="col-span-full py-20 flex justify-center"><Loader2 size={48} className="animate-spin text-indigo-500" /></div>
          ) : (
              plans.map((plan, index) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="mb-6">
                            <h3 className="text-lg font-bold text-slate-400 uppercase tracking-widest mb-2">{plan.name}</h3>
                            <div className="flex items-baseline gap-1">
                                <span className="text-4xl font-black text-white">
                                    {isEnterprise ? 'R$ 697' : isPopular ? 'R$ 397' : 'R$ 197'}
                                </span>
                        ...
                        <ul className="space-y-4 mb-8 flex-1">
                            <li className="flex items-center gap-3 text-slate-300 text-sm">
                                <div className="p-1 bg-emerald-500/10 rounded-full text-emerald-500"><Check size={12} strokeWidth={3} /></div>
                                <span>{plan.users === 0 ? 'Usuários Ilimitados' : `${plan.users} Usuários`}</span>
                            </li>
                            <li className="flex items-center gap-3 text-slate-300 text-sm">
                                <div className="p-1 bg-emerald-500/10 rounded-full text-emerald-500"><Check size={12} strokeWidth={3} /></div>
                                <span>{plan.connections === 0 ? 'Conexões Ilimitadas' : `${plan.connections} WhatsApps`}</span>
                            </li>
                            <li className="flex items-center gap-3 text-slate-300 text-sm">
                                <div className="p-1 bg-emerald-500/10 rounded-full text-emerald-500"><Check size={12} strokeWidth={3} /></div>
                                <span>{plan.queues === 0 ? 'Setores Ilimitados' : `${plan.queues} Setores`}</span>
                            </li>
                            
                            {plan.useCampaigns && (
                                <li className="flex items-center gap-3 text-white font-bold text-sm">
                                    <div className="p-1 bg-indigo-500/20 rounded-full text-indigo-400"><Zap size={12} strokeWidth={3} /></div>
                                    Campanhas em Massa
                                </li>
                            )}
                            {plan.useIntegrations && (
                                <li className="flex items-center gap-3 text-white font-bold text-sm">
                                    <div className="p-1 bg-indigo-500/20 rounded-full text-indigo-400"><Rocket size={12} strokeWidth={3} /></div>
                                    Integração IXC/VoIP
                                </li>
                            )}
                            {plan.useOpenAi && (
                                <li className="flex items-center gap-3 text-white font-bold text-sm">
                                    <div className="p-1 bg-purple-500/20 rounded-full text-purple-400"><Crown size={12} strokeWidth={3} /></div>
                        ...
                        <button 
                            ...
                            `}
                        >
                            {processingId === plan.id ? <Loader2 size={20} className="animate-spin" /> : 
                             paymentMethod === 'card' ? <CreditCard size={20} /> : <FileText size={20} />}
                            
                            {processingId === plan.id ? 'Processando...' : 
                             paymentMethod === 'card' ? 'Assinar com Cartão' : 'Gerar Fatura Pix'}
                        </button>
                    </div>
                  );
              })
      ...
      <div className="text-center pt-12 border-t border-slate-800">
          ...
          <p className="text-slate-600 text-xs">
              Dúvidas? Entre em contato com nosso <a href="#" className="text-indigo-500 hover:underline">suporte</a>.
          </p>
      </div>
