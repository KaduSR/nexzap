...
const Incidents: React.FC = () => {
  ...
  const [newIncident, setNewIncident] = useState({ title: '', description: '', tagId: '' });
  ...
  const handleToggle = async (incident: Incident) => {
      // Optimistic Update
      const updatedIncidents = incidents.map(i => i.id === incident.id ? { ...i, isActive: !i.isActive } : i);
  ...
  const displayTags = tags.length > 0 ? tags : [{id: 1, name: 'Centro'}, {id: 2, name: 'Zona Norte'}, {id: 3, name: 'Fibra Óptica'}];
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">
             Ative o "Modo Pânico" para bloquear a criação massiva de tickets durante instabilidades.
          </p>
        </div>
        <button onClick={fetchData} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-slate-400">
      ...
      <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-[32px] p-8 shadow-sm">
         ...
         <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-12 gap-4">
             <div className="md:col-span-3 space-y-1">
                 <label className="text-[10px] font-black uppercase text-red-400 pl-1">Título Interno</label>
                 <input 
                    type="text" 
             ...
             <div className="md:col-span-3 space-y-1">
                 <label className="text-[10px] font-black uppercase text-red-400 pl-1">Região Afetada (Tag)</label>
                 <select 
                    required
                    value={newIncident.tagId}
                    onChange={e => setNewIncident({...newIncident, tagId: e.target.value})}
             ...
             <div className="md:col-span-4 space-y-1">
                 <label className="text-[10px] font-black uppercase text-red-400 pl-1">Mensagem Automática para o Cliente</label>
                 <input 
                    type="text" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {incidents.length === 0 ? (
              ...
              incidents.map(incident => (
                  <div key={incident.id} className={`p-6 rounded-[24px] border-2 relative overflow-hidden transition-all ${incident.isActive ? 'bg-red-50 dark:bg-red-900/20 border-red-500 shadow-xl shadow-red-500/10' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 opacity-70'}`}>
                      <div className="flex justify-between items-start mb-4 relative z-10">
                          <div>
                              <h3 className={`font-black text-lg ${incident.isActive ? 'text-red-700 dark:text-red-400' : 'text-slate-700 dark:text-slate-300'}`}>
                                  {incident.title}
                              </h3>
                          ...
                          <button 
                            ...
                            className={`w-12 h-8 rounded-full flex items-center transition-colors px-1 ${incident.isActive ? 'bg-red-500 justify-end' : 'bg-slate-300 dark:bg-slate-600 justify-start'}`}
                          >
