...
import { 
  Search, MoreVertical, Send, Paperclip, Smile, CheckCheck, User, Phone, Info, Mic, 
  Image as ImageIcon, MessageSquare, Database, FileText, CreditCard, History, 
  AlertTriangle, Copy, Check, Plus, X, Wrench, ChevronDown, PhoneCall, PhoneOff, 
  MicOff, Delete, Grid, Zap, Lock, Unlock, RefreshCw, Loader2, ArrowLeft, Filter,
  MoreHorizontal, LockOpen, Clock, Calendar, Sparkles, Tag, ArrowRightLeft
} from 'lucide-react';
...
const DUMMY_TICKETS: Ticket[] = [
  { id: '1', name: 'João Silva', lastMsg: 'Segue o comprovante...', time: '10:45', unread: 2, status: 'open', ixcCustomer: true, address: 'Rua das Flores, 123', number: '5511999887766' },
  { id: '2', name: 'Maria Souza', lastMsg: 'Obrigado!', time: 'Ontem', unread: 0, status: 'pending', ixcCustomer: false, address: 'Av. Paulista, 1000', number: '5511988776655' },
  { id: '3', name: 'Empresa Alpha', lastMsg: 'Atendimento finalizado com sucesso.', time: '12/10', unread: 0, status: 'closed', ixcCustomer: true, address: 'Rua Empresarial, 500', number: '5511977665544' },
];

const INITIAL_MESSAGES: Message[] = [
  { id: 'm1', fromMe: false, text: 'Olá, como posso ajudar?', time: '10:40' },
  { id: 'm2', fromMe: true, text: 'Gostaria de ver minha fatura de internet.', time: '10:42' },
  { id: 'm3', fromMe: false, text: 'Claro! Aqui está o áudio explicando como acessar o portal:', time: '10:44' },
  { id: 'm4', fromMe: false, mediaType: 'audio', mediaUrl: 'https://wavesurfer.xyz/wavesurfer-code/examples/audio/audio.wav', time: '10:44' },
  { id: 'm5', fromMe: true, text: 'Nota Interna: Cliente reclama de lentidão recorrente. Verificar sinal.', time: '10:46', isPrivate: true },
];
...
const Tickets: React.FC = () => {
  ...
  const [currentMessages, setCurrentMessages] = useState<Message[]>(INITIAL_MESSAGES);
  ...
  const [osData, setOsData] = useState({ subject: '1', department: '101', description: '' });
  ...
  const [unlockStatus, setUnlockStatus] = useState<'idle' | 'loading' | 'success'>('idle');
      ...
      try {
        ...
        if (res.ok) {
            ...
            if (Array.isArray(data)) {
              const setting = data.find((s: any) => s.key === 'ai_auto_transcribe_audio');
  ...
  const fetchIxcData = async () => {
      ...
      try {
          ...
          setIxcInvoices([
              { id: 101, vencimento: '10/11/2024', valor: '99,90', status: 'Aberto', link: '#', pix: '00020126580014br.gov.bcb.pix0136123e4567-e89b-12d3-a456-426614174000520400005303986540599.905802BR5913Internet Provider6008Sao Paulo' },
              { id: 102, vencimento: '10/10/2024', valor: '99,90', status: 'Pago', link: '#', pix: '' },
          ]);
      } catch (error) {
  ...
  const handleUnlock = async () => {
      if (!confirm("Confirmar desbloqueio de confiança para este cliente?")) return;
      setUnlockStatus('loading');
      setTimeout(() => {
          setUnlockStatus('success');
          alert("Desbloqueio realizado com sucesso! O cliente deve reiniciar o equipamento.");
  ...
  const handleScheduleMessage = async (e: React.FormEvent) => {
      e.preventDefault();
      if (!selectedTicket || !message.trim() || !scheduleDate || !scheduleTime) return;

      setIsScheduling(true);
      try {
          const sendAt = `${scheduleDate}T${scheduleTime}:00.000Z`; // Simple ISO format construction
          
          const response = await fetch(`${API_URL}/api/tickets/${selectedTicket.id}/schedules`, {
              method: 'POST',
              headers: { 
                  'Content-Type': 'application/json',
                  'Authorization': 'Bearer token'
  ...
  const filteredTickets = DUMMY_TICKETS.filter(t => t.status === statusFilter);
      ...
      `}>
        ...
        <div className="p-6 space-y-5">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black tracking-tight text-slate-800 dark:text-white">Atendimento</h2>
          ...
          <div className="flex gap-1 p-1.5 bg-slate-100 dark:bg-slate-800/50 rounded-2xl">
            {['open', 'pending', 'closed'].map((status) => (
                ...
                >
                    {status === 'open' ? 'Abertos' : status === 'pending' ? 'Fila' : 'Resolvidos'}
                </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-3 pb-3 space-y-1">
          {filteredTickets.length === 0 ? (
             <div className="flex flex-col items-center justify-center h-64 text-center opacity-50 p-8">
                ...
                <p className="text-sm font-bold text-slate-600 dark:text-slate-400">Nenhum ticket encontrado.</p>
                <p className="text-xs text-slate-400 mt-1">Aguardando novos contatos.</p>
             </div>
          ) : (
            filteredTickets.map(ticket => (
              ...
              >
                <div className="relative shrink-0">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center overflow-hidden border-2 ${selectedTicket?.id === ticket.id ? 'border-white/20' : 'border-slate-200 dark:border-slate-700'}`}>
                    <img src={`https://picsum.photos/seed/${ticket.id}/100/100`} alt={ticket.name} className="w-full h-full object-cover" />
                  </div>
                  {ticket.ixcCustomer && (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start mb-1">
                    <h3 className={`font-bold text-sm truncate ${selectedTicket?.id === ticket.id ? 'text-white' : 'text-slate-800 dark:text-white'}`}>{ticket.name}</h3>
                    <span className={`text-[10px] font-bold whitespace-nowrap ml-2 ${selectedTicket?.id === ticket.id ? 'text-indigo-200' : 'text-slate-400'}`}>{ticket.time}</span>
                  </div>
                  <p className={`text-xs truncate leading-relaxed ${selectedTicket?.id === ticket.id ? 'text-indigo-100' : 'text-slate-500 dark:text-slate-400'}`}>
                    {ticket.lastMsg}
                  </p>
                </div>
                {ticket.unread > 0 && (
                  <div className={`text-[10px] font-black w-6 h-6 flex items-center justify-center rounded-full self-center shadow-sm ${
                      selectedTicket?.id === ticket.id 
        ...
        {selectedTicket ? (
          <>
            ...
            <header className="px-6 py-4 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 flex items-center justify-between z-20 sticky top-0">
              <div className="flex items-center gap-4">
                 {isMobile && (
                    <button onClick={() => setSelectedTicket(null)} className="p-2 -ml-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800">
                        <ArrowLeft size={20} />
                    </button>
                 )}
                <div className="w-11 h-11 rounded-2xl bg-indigo-100 dark:bg-slate-800 overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700">
                  <img src={`https://picsum.photos/seed/${selectedTicket.id}/100/100`} alt="" className="w-full h-full object-cover" />
                </div>
                <div>
                  <h2 className="font-bold text-base text-slate-800 dark:text-white leading-tight">{selectedTicket.name}</h2>
                  <div className="flex items-center gap-2 mt-0.5">
                     ...
                     {selectedTicket.ixcCustomer && (
                         <span className="text-[10px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-md">Assinante</span>
                     )}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {isResolving ? <Loader2 size={16} className="animate-spin" /> : <CheckCheck size={16} />}
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 custom-scrollbar bg-slate-50 dark:bg-slate-950">
              <div className="flex justify-center my-4">
                <span className="text-[10px] font-bold px-4 py-1.5 bg-slate-200 dark:bg-slate-800 rounded-full text-slate-500 dark:text-slate-400 uppercase tracking-widest shadow-sm border border-slate-300 dark:border-slate-700">Hoje</span>
              </div>
              
              {currentMessages.map(msg => (
                <div key={msg.id} className={`flex ${msg.fromMe ? 'justify-end' : 'justify-start'} group animate-in slide-in-from-bottom-2 duration-300`}>
                  {/* Avatar for receiver */}
                  {!msg.fromMe && !msg.isPrivate && (
                      <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 mr-2 mt-auto overflow-hidden hidden sm:block">
                          <img src={`https://picsum.photos/seed/${selectedTicket.id}/100/100`} className="w-full h-full" alt="" />
                      </div>
                  )}

                  <div className={`max-w-[85%] md:max-w-[65%] relative ${msg.fromMe ? 'order-2' : ''}`}>
                    {msg.isPrivate && (
                        <div className="flex items-center gap-1 text-[10px] text-amber-500 font-bold mb-1 justify-end">
                            <Lock size={10} /> Nota Interna (Invisível para o cliente)
                        </div>
                    )}
                    <div className={`p-4 rounded-3xl shadow-sm relative text-sm ${
                      msg.isPrivate
                      ...
                      {msg.mediaType === 'audio' ? (
                          <div className="py-1">
                              <AudioPlayer src={msg.mediaUrl || ''} autoTranscribe={autoTranscribeAudio} />
                          </div>
                      ) : (
                    ...
                    <div className={`flex items-center gap-1 mt-1 px-1 ${msg.fromMe ? 'justify-end' : 'justify-start'}`}>
                        <span className="text-[10px] font-bold text-slate-400">{msg.time}</span>
                        {msg.fromMe && !msg.isPrivate && <CheckCheck size={12} className="text-indigo-500" />}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {/* Input Footer */}
            <footer className={`p-4 md:p-6 bg-slate-50 dark:bg-slate-950 z-20 ${isPrivateMode ? 'border-t-2 border-amber-400' : ''}`}>
              <div className={`
              ...
              `}>
                <button 
                    ...
                    className={`p-3 rounded-xl transition-colors ${isPrivateMode ? 'bg-amber-100 text-amber-600' : 'text-slate-400 hover:text-indigo-600 hover:bg-slate-50 dark:hover:bg-slate-800'}`}
                ...
                <div className="flex-1 py-3 max-h-32 overflow-y-auto custom-scrollbar">
                    <textarea 
                        placeholder={isPrivateMode ? "Escrever nota interna (cliente não verá)..." : "Digite sua mensagem..."} 
                        className={`w-full bg-transparent border-none outline-none text-sm font-medium resize-none placeholder-slate-400 h-6 max-h-24 ${isPrivateMode ? 'text-amber-800 dark:text-amber-200 placeholder-amber-400/70' : 'text-slate-800 dark:text-slate-200'}`}
                        value={message}
                ...
                <div className="flex gap-1 pb-1">
                    ...
                    {message.trim() ? (
                        <button 
                            ...
                            className={`p-2.5 text-white rounded-xl shadow-lg transition-all hover:scale-105 active:scale-95 ${isPrivateMode ? 'bg-amber-500 shadow-amber-500/30' : 'bg-indigo-600 shadow-indigo-500/30'}`}
                        >
              ...
              {isPrivateMode && (
                  <div className="text-center mt-2 text-[10px] font-bold text-amber-500 flex items-center justify-center gap-1 animate-pulse">
                      <AlertTriangle size={10} /> MODO PRIVADO ATIVO: O cliente NÃO receberá esta mensagem.
                  </div>
              )}
            </footer>
          </>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 space-y-6 animate-in zoom-in-95 duration-500">
            ...
            <div className="text-center space-y-2">
                <h3 className="text-xl font-bold text-slate-700 dark:text-slate-200">Nenhuma conversa selecionada</h3>
                <p className="text-sm font-medium text-slate-500 max-w-xs mx-auto">Selecione um ticket na lista lateral para iniciar o atendimento.</p>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900">
                    ...
                    <button onClick={() => setIsScheduleModalOpen(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl text-slate-400">
                        <X size={20} />
                    </button>
                </div>
        ...
        `}>
          ...
          <div className="p-6 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 shrink-0">
             <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => setShowIxcPanel(false)} className="lg:hidden p-1 bg-slate-200 dark:bg-slate-800 rounded-lg text-slate-500">
                    <X size={16} />
                </button>
             </div>
             ...
             <div className="flex flex-col items-center text-center">
                <div className="w-20 h-20 rounded-2xl bg-white dark:bg-slate-800 p-1 shadow-lg mb-3">
                    <img src={`https://picsum.photos/seed/${selectedTicket.id}/100/100`} className="w-full h-full rounded-xl object-cover" alt="" />
                </div>
                <h3 className="font-black text-xl leading-tight text-slate-800 dark:text-white">{selectedTicket.name}</h3>
                <p className="text-slate-500 text-xs font-mono mt-1 bg-slate-200 dark:bg-slate-800 px-2 py-1 rounded-md">CPF: 123.***.***-00</p>
             </div>
          </div>
          ...
          <div className="p-6 space-y-8 flex-1">
            ...
            <div className="space-y-3">
               <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Status da Conexão</h4>
               <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-3">
                      ...
                      <div>
                          <p className="font-bold text-sm text-emerald-700 dark:text-emerald-400">Conectado</p>
                          <p className="text-[10px] text-emerald-600 dark:text-emerald-500 font-medium">Fibra 500MB • PPPoE</p>
                      </div>
                  </div>
               ...
               <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    {unlockStatus === 'loading' ? <Loader2 size={18} className="animate-spin text-indigo-600" /> : <LockOpen size={18} className="text-indigo-500" />}
                    Desbloqueio
                  </button>
            ...
            <div className="space-y-3">
                <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest flex justify-between items-center">
                    Faturas em Aberto
                    <span className="text-[10px] bg-slate-200 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded">{ixcInvoices.length}</span>
                </h4>
                ...
                {loadingIxc ? (
                    ...
                    <div className="space-y-2">
                        {ixcInvoices.map(inv => (
                            <div key={inv.id} className="p-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl flex flex-col gap-2">
                                <div className="flex justify-between items-center">
                                    <span className="font-bold text-sm text-slate-700 dark:text-slate-200">R$ {inv.valor}</span>
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${inv.status === 'Pago' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                                        {inv.status}
                                    </span>
                                </div>
                                <div className="flex justify-between items-center text-xs text-slate-500">
                                    ...
                                    <div className="flex gap-2">
                                        <button className="hover:text-indigo-600" title="Ver Boleto"><FileText size={14}/></button>
                                        {inv.pix && (
                                            <button 
                                                onClick={() => handleCopyPix(inv.pix)}
                                                className={`hover:text-emerald-500 transition-colors ${copiedPix === inv.pix ? 'text-emerald-500' : ''}`} 
                                                title="Copiar Pix"
                                            >
                                                {copiedPix === inv.pix ? <Check size={14}/> : <Copy size={14}/>}
                                            </button>
                                        )}
                                    </div>
                                </div>
      ...
      {showOsModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900">
                    <h3 className="font-black text-lg flex items-center gap-2">
                        <Wrench className="text-indigo-600" size={20}/> Nova Ordem de Serviço
                    </h3>
                    <button onClick={() => setShowOsModal(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl text-slate-400">
                        <X size={20} />
                    </button>
                </div>
                <form onSubmit={handleCreateOS} className="p-6 space-y-4">
                    <div className="space-y-2">
                        ...
                        <select 
                            ...
                            onChange={e => setOsData({...osData, subject: e.target.value})}
                        >
                    ...
                    <div className="space-y-2">
                        ...
                        <select 
                            ...
                            onChange={e => setOsData({...osData, department: e.target.value})}
                        >
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea 
                            ...
                            onChange={e => setOsData({...osData, description: e.target.value})}
                            required
                        />
                    </div>
                    <button 
                        ...
                        className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all"
                    >
                        {isCreatingOs ? <Loader2 size={18} className="animate-spin" /> : <Check size={18} />}
                        Abrir O.S.
                    </button>
                </form>
            </div>
