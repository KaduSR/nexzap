...
const Softphone: React.FC = () => {
  ...
  const filteredCalls = recentCalls.filter(call => 
      call.number.includes(searchTerm) || (call.name && call.name.toLowerCase().includes(searchTerm.toLowerCase()))
  );

  const getCallIcon = (type: CallLog['type']) => {
      switch(type) {
          case 'incoming': return <PhoneIncoming size={16} className="text-blue-500" />;
          case 'outgoing': return <PhoneOutgoing size={16} className="text-emerald-500" />;
  ...
  return (
    <div className="h-full flex bg-slate-950 text-white animate-in fade-in duration-500">
        ...
        <div className="w-96 border-r border-slate-800 flex flex-col bg-slate-900">
            <div className="p-6 border-b border-slate-800">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-slate-500">
                    <div className={`w-2 h-2 rounded-full ${connectionStatus === 'Registered' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-red-500'}`}></div>
                    {connectionStatus === 'Registered' ? 'Linha Pronta' : 'Desconectado'}
                </div>
            </div>
                        ...
                        {filteredCalls.map((call) => (
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className={`text-sm font-bold ${call.type === 'missed' ? 'text-red-400' : 'text-slate-200'}`}>
                                            {call.name || call.number}
                                        </p>
                                        <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                            {getCallIcon(call.type)}
                                            <span>{format(new Date(call.date), 'dd/MM HH:mm')}</span>
                                        </div>
                                    </div>
                                </div>
        ...
        {/* RIGHT PANEL: DIALER */}
        <div className="flex-1 flex flex-col items-center justify-center p-8 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-slate-950 to-slate-950">
                ...
                <div className="mb-8 text-center relative">
                    {callStatus === 'Idle' ? (
                        ...
                        <div className="animate-in zoom-in duration-300">
                            ...
                            <h2 className="text-3xl font-bold text-white mb-1">{dialNumber}</h2>
                            <p className="text-emerald-400 font-mono text-lg font-medium">{formatDuration(callDuration)}</p>
                        </div>
                    )}
                </div>
                    ...
                    {callStatus === 'Idle' ? (
                        ...
                        <>
                            <button 
                                ...
                                className={`w-16 h-16 rounded-full flex items-center justify-center transition-all ${isMuted ? 'bg-white text-slate-900' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                            >
