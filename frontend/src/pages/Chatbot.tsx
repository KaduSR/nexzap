...
const Chatbot: React.FC = () => {
  ...
  const addNode = (type: string, label: string) => {
    ...
    if (type === "message")
      defaultData = { content: "Nova mensagem...", nextId: "" };
    if (type === "choice")
      defaultData = {
        content: "Escolha uma opção:",
        options: [{ value: "1", label: "Opção 1", nextId: "" }],
      };
    ...
    if (
      lastNode &&
      !["choice", "condition", "transfer"].includes(lastNode.type)
    ) {
  ...
  const addOptionToChoice = () => {
    ...
    const updatedData = {
      ...selectedNode.data,
      options: [...(selectedNode.data.options || []), newOption],
    };
    setNodes(
      nodes.map((n) =>
    ...
    setNodes(
      nodes.map((n) =>
        n.id === selectedNode.id
          ? { ...n, data: { ...selectedNode.data, options: newOptions } }
          : n
      )
    );
  };
  ...
  const removeOption = (index: number) => {
    ...
    setNodes(
      nodes.map((n) =>
        n.id === selectedNode.id
          ? { ...n, data: { ...n.data, options: newOptions } }
          : n
      )
    );
  };
        ...
        <div className="flex items-center gap-10 w-full">
          ...
          <div className="hidden sm:block">
            <div className="flex items-center gap-2">
              ...
              <span className="bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-widest">
                PRO
              </span>
            </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          >
            ...
            <span className="hidden md:inline">
              {isSaving ? "Salvando..." : "Publicar Fluxo"}
            </span>
          </button>
        </div>
      </header>
        ...
        <main className="flex-1 relative bg-[radial-gradient(#e2e8f0_1.5px,transparent_1.5px)] dark:bg-[radial-gradient(#1e293b_1.5px,transparent_1.5px)] bg-size-[40px_40px] overflow-auto custom-scrollbar">
          <div className="min-w-200 min-h-375 p-8 md:p-24 flex flex-col items-center relative">
              ...
              <React.Fragment key={node.id}>
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[9px] bg-slate-100 dark:bg-slate-800 px-1.5 rounded text-slate-500 font-mono">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400 font-medium">
                    {node.type === "message" && (
                      <p className="line-clamp-2">"{node.data.content}"</p>
                    )}
                    {node.type === "input" && (
                      <p className="line-clamp-2">
                        Q: "{node.data.content}" &rarr; Salvar em:{" "}
                        <b>{node.data.variable}</b>
                      </p>
                    )}
                    {node.type === "wait" && (
                      <p>
                        Aguardar <b>{node.data.seconds}</b> segundos.
                      </p>
                    )}
                    {node.type === "tag" && (
                      <p>
                        Tag: <b>{node.data.tag}</b>
                      </p>
                    )}
                    {node.type === "condition" && (
                      <p>
                        Se <b>{node.data.variable}</b> {node.data.operator} ...
                      </p>
                    )}
                    {node.type === "choice" && (
                    ...
                    {node.type === "trigger" && (
                      <p>Gatilho: {node.data.trigger}</p>
                    )}
                    {node.type === "transfer" && (
                      <p>Fila ID: {node.data.queueId}</p>
                    )}
                  </div>

                  {node.data.nextId &&
                    !["choice", "condition"].includes(node.type) && (
                      <div className="mt-3 flex items-center justify-center gap-1 text-[10px] text-indigo-500 font-bold bg-indigo-50 dark:bg-indigo-900/20 py-1 rounded-lg">
                        <ArrowDown size={12} /> Vai para{" "}
                        {node.data.nextId.split("_")[1]}
                      </div>
                    )}
                </div>
            ...
            <button
              ...
              className="mt-8 p-4 bg-white dark:bg-slate-800 rounded-full shadow-lg text-indigo-600 hover:scale-110 transition-transform"
            >
              <Plus />
            </button>
          </div>
            ...
            >
              ...
              <div className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-6">
                ...
                {selectedNode.type === "input" && (
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase">
                      Nome da Variável (Salvar Resposta)
                    </label>
                    <input
                      type="text"
                        ...
                        setNodes(
                          nodes.map((n) =>
                            n.id === selectedNode.id
                              ? {
                                  ...n,
                                  data: { ...n.data, tag: e.target.value },
                                }
                              : n
                          )
                        )
                ...
                {selectedNode.type === "condition" && (
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-100 dark:border-red-900/30">
                      <label className="text-xs font-bold text-red-600 dark:text-red-400 uppercase">
                        Se FALSO, ir para:
                      </label>
                      <select
                ...
                {selectedNode.type === "choice" && (
                  <div className="space-y-4">
                    ...
                    {selectedNode.data.options?.map((opt: any, idx: number) => (
                      <div
                        key={idx}
                        className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3 relative group"
                      >
                        <button
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <select
                            ...
                            value={opt.nextId}
                            onChange={(e) =>
                              updateOption(idx, "nextId", e.target.value)
                            }
                          >
                        ...
                        setNodes(
                          nodes.map((n) =>
                            n.id === selectedNode.id
                              ? {
                                  ...n,
                                  data: { ...n.data, nextId: e.target.value },
                                }
                              : n
                          )
                        )
