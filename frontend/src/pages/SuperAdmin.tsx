  ...
  const fetchData = async () => {
    ...
    try {
        const [compRes, planRes] = await Promise.all([
            fetch(`${API_URL}/api/companies`, { headers: { 'Authorization': 'Bearer token' } }),
            fetch(`${API_URL}/api/companies/plans`, { headers: { 'Authorization': 'Bearer token' } })
        ]);
  ...
  const handleSave = async (e: React.FormEvent) => {
      ...
      try {
          ...
          if (res.ok) {
              setShowModal(false);
              fetchData();
              alert(isEdit ? "Empresa atualizada com sucesso!" : "Nova empresa criada com sucesso!");
          } else {
    ...
    <div className="p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Gerencie planos, clientes e assinaturas do seu SaaS.</p>
        </div>
      ...
      <div className="flex flex-wrap gap-4 items-center bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <div className="flex items-center gap-3 px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl">
                ...
                <div>
                    <p className="text-[10px] text-slate-500 uppercase font-black">Total Clientes</p>
                    <span className="font-bold text-lg text-slate-800 dark:text-white">{companies.length}</span>
                </div>
            </div>
            <div className="flex items-center gap-3 px-4 py-2 bg-emerald-50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-900/20">
                ...
                <div>
                    <p className="text-[10px] text-emerald-600 uppercase font-black">Ativos</p>
                    <span className="font-bold text-lg text-emerald-700 dark:text-emerald-400">{companies.filter(c => c.status).length}</span>
                </div>
            </div>
            <div className="flex items-center gap-3 px-4 py-2 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/20">
                ...
                <div>
                    <p className="text-[10px] text-red-600 uppercase font-black">Bloqueados</p>
                    <span className="font-bold text-lg text-red-700 dark:text-red-400">{companies.filter(c => !c.status).length}</span>
                </div>
            </div>
      </div>
          ...
          <div className="overflow-x-auto">
              <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {loading ? (
                          <tr><td colSpan={6} className="p-12 text-center"><Loader2 className="animate-spin mx-auto text-indigo-500" /></td></tr>
                      ) : companies.map(company => (
                          <tr key={company.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                              <td className="p-6">
                                  <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                          <p className="font-bold text-slate-800 dark:text-white text-sm">{company.name}</p>
                                          <p className="text-[10px] text-slate-500 font-mono flex items-center gap-1">
                                              <FileText size={10} /> {company.document || 'Sem Doc'}
                                          </p>
                                      </div>
                                  </div>
                              </td>
                              ...
                              <td className="p-6">
                                  <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-bold uppercase tracking-wider ${
                                      company.plan.name.includes('Enterprise') ? 'bg-purple-100 text-purple-600 border-purple-200 dark:bg-purple-900/20 dark:border-purple-800' :
                                      company.plan.name.includes('Pro') ? 'bg-indigo-100 text-indigo-600 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-800' :
                                      'bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:border-slate-700'
                                  }`}>
                                      <Zap size={12} fill="currentColor" /> {company.plan.name}
                                  </span>
                              </td>
                              <td className="p-6">
            ...
            <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-black text-lg">{form.id ? 'Editar Cliente' : 'Novo Cliente'}</h3>
                            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">{form.id ? form.name : 'Cadastro de Tenant'}</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSave} className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Razão Social / Nome</label>
                                <input 
                                    type="text" 
                                    required
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Email Financeiro</label>
                                <input 
                                    type="email" 
                                    required
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm outline-none focus:border-indigo-500"
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            <div className="col-span-3 space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Logradouro Completo</label>
                                <input 
                                    type="text" 
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm outline-none focus:border-indigo-500"
                                    value={form.address}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-slate-500 uppercase">Plano Selecionado</label>
                                <select 
                                    className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm outline-none focus:border-indigo-500"
                                    value={form.planId}
                        ...
                        <div 
                            className={`flex items-center gap-4 p-4 rounded-xl border cursor-pointer transition-all ${form.status ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-800' : 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-800'}`}
                            onClick={() => setForm({...form, status: !form.status})}
                        >
                            <div className={`w-12 h-6 rounded-full relative transition-colors ${form.status ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-600'}`}>
                                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${form.status ? 'left-7' : 'left-1'}`}></div>
                            </div>
                            <div>
                                <span className={`block font-bold text-sm ${form.status ? 'text-emerald-700 dark:text-emerald-400' : 'text-red-700 dark:text-red-400'}`}>
                                    {form.status ? 'Acesso Liberado' : 'Acesso Bloqueado'}
                                </span>
                                <span className="text-[10px] text-slate-500">
                                    {form.status ? 'O cliente pode acessar o sistema.' : 'O cliente verá uma tela de bloqueio.'}
                                </span>
                            </div>
                        </div>
                    </div>
