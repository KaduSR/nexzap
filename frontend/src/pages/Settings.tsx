...
const AI_MODELS = {
  openai: {
    ...
    transcription: [{ id: "whisper-1", name: "Whisper 1" }],
  },
  gemini: {
    response: [
  ...
  groq: {
    ...
    transcription: [{ id: "whisper-large-v3", name: "Whisper Large v3" }],
  },
};
...
const Settings: React.FC = () => {
  ...
  const [currentQm, setCurrentQm] = useState<Partial<QuickMessage>>({
    shortcode: "",
    message: "",
  });
    ...
    try {
      ...
      if (Array.isArray(data)) {
        const getValue = (key: string, defaultValue: any) =>
          data.find((s: any) => s.key === key)?.value ?? defaultValue;
        ...
        setAiConfig({
          ...
          autoTranscribeAudio:
            getValue("ai_auto_transcribe_audio", "false") === "true",
          transcriptionProvider: getValue(
            "ai_transcription_provider",
            "gemini"
          ),
          transcriptionApiKey: getValue("ai_transcription_api_key", ""),
          transcriptionModel: getValue(
  ...
  const handleSaveAi = async () => {
    ...
    try {
      const settingsToSave = [
        ...
        {
          key: "ai_auto_transcribe_audio",
          value: String(aiConfig.autoTranscribeAudio),
        },
        {
          key: "ai_transcription_provider",
          value: aiConfig.transcriptionProvider,
        },
        {
          key: "ai_transcription_api_key",
          value: aiConfig.transcriptionApiKey,
        },
  ...
  const handleSaveNotifications = async () => {
    ...
    try {
      const settingsToSave = [
        ...
        {
          key: "notify_ticket_close",
          value: String(notifications.ticketClose),
        },
        {
          key: "notify_connection",
          value: String(notifications.connectionAlert),
        },
      ];
      for (const setting of settingsToSave) {
        await fetch(`${API_URL}/api/settings/${setting.key}`, {
  ...
  const settingsGroups = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          description: "Atalhos de mensagens (/)",
          action: () => setActiveSection("quick_messages"),
        },
        {
          icon: <Brain className="text-violet-500" />,
          label: "Inteligência Artificial",
          description: "Chatbot LLM (Gemini, OpenAI, Groq)",
          action: () => setActiveSection("ai"),
        },
        {
          icon: <Palette className="text-pink-500" />,
          label: "Cores & Marca",
          description: "Logotipo e cores personalizadas",
          action: () => setActiveSection("branding"),
        },
        {
          icon: <Bell className="text-teal-500" />,
          label: "Notificações",
          description: "Configurar avisos do sistema",
      ...
      items: [
        {
          ...
          description: "Gerenciar contatos e dados",
          action: () => setActiveSection("data"),
        },
        {
          icon: <Shield className="text-red-500" />,
          label: "Segurança & Logs",
          description: "Registros de acesso e auditoria",
      ...
      case "branding":
        return (
          <div className="p-8 max-w-2xl mx-auto animate-in slide-in-from-right duration-300">
            <SettingsHeader
              title="Cores & Marca"
              desc="Personalize a aparência do sistema."
            />
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl border dark:border-slate-700 space-y-6">
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase">
              ...
              <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <button className="px-3 py-1.5 bg-white dark:bg-slate-800 border dark:border-slate-600 rounded-lg text-xs font-bold">
                  Upload
                </button>
              </div>
              <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700">
      ...
      case "notifications":
        return (
          <div className="p-8 max-w-2xl mx-auto animate-in slide-in-from-right duration-300">
            <SettingsHeader
              title="Notificações"
              desc="Controle como você recebe alertas."
            />
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl border dark:border-slate-700 space-y-4">
              {[
                {
                  key: "newTicket",
                  label: "Novos Tickets",
                  desc: "Receber alerta quando um cliente abrir um ticket.",
                },
                {
                  key: "newMessage",
                ...
                },
                {
                  ...
                  desc: "Notificar quando um ticket for encerrado.",
                },
                {
                  key: "connectionAlert",
                  label: "Alertas de Conexão",
                  desc: "Avisar se o WhatsApp desconectar.",
                },
              ].map((item) => (
                <div
                  key={item.key}
                  className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer"
                  onClick={() =>
                    setNotifications({
                      ...notifications,
                      [item.key]:
              ...
              <button
                ...
                disabled={isSaving}
                className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 mt-4"
              >
                {isSaving ? <Loader2 className="animate-spin" /> : <Save />}{" "}
                Salvar Preferências
              </button>
        ...
        return (
          <div className="p-8 max-w-2xl mx-auto animate-in slide-in-from-right duration-300">
            <SettingsHeader
              ...
              desc="Gerencie seus dados e backups."
            />
      ...
      case "security":
        return (
          <div className="p-8 max-w-2xl mx-auto animate-in slide-in-from-right duration-300">
            <SettingsHeader
              title="Segurança"
              desc="Proteja sua conta e monitore acessos."
            />
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl border dark:border-slate-700 space-y-6">
          ...
          <div className="p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
            <div>
              ...
              <p className="text-slate-500">
                Personalize o Whaticket Plus conforme sua necessidade.
              </p>
            </div>
            ...
            <div className="space-y-8">
              {settingsGroups.map((group) => (
                <div key={group.title} className="space-y-4">
                  <h2 className="text-xs font-bold uppercase tracking-widest text-slate-400 pl-4">
                    {group.title}
                  </h2>
            ...
            <div className="mt-12 p-6 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-100 dark:border-red-900/30">
              ...
              <button
                onClick={() =>
                  confirm(
                    "Tem certeza? Esta ação deletará todos os seus dados."
                  ) && alert("Solicitação de exclusão enviada.")
                }
                className="text-red-600 text-sm font-bold border border-red-200 dark:border-red-900/50 px-4 py-2 rounded-xl hover:bg-red-600 hover:text-white transition-all"
              >
