...
const Queues: React.FC = () => {
  ...
  const [currentQueue, setCurrentQueue] = useState<Partial<Queue>>({ name: '', color: '#6366f1', greetingMessage: '' });
  ...
  const fetchData = async () => {
    ...
    try {
      const [queuesRes, usersRes] = await Promise.all([
          fetch(`${API_URL}/api/queues`, { headers: { 'Authorization': 'Bearer token' } }),
  ...
  const handleSave = async (e: React.FormEvent) => {
      ...
      try {
          ...
          const url = currentQueue.id 
            ? `${API_URL}/api/queues/${currentQueue.id}` 
            : `${API_URL}/api/queues`;
  ...
  const handleDelete = async (id: number) => {
      if (!confirm("Tem certeza que deseja excluir este setor? Isso pode afetar tickets abertos.")) return;
      try {
    ...
    <div className="p-8 space-y-8 max-w-6xl mx-auto animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">Vincule atendentes aos departamentos para roteamento automático.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
              ...
              queues.map(queue => (
                  <div key={queue.id} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm relative group hover:border-indigo-500 transition-colors flex flex-col">
                      <div className={`absolute top-0 left-0 w-1.5 h-full rounded-l-2xl`} style={{backgroundColor: queue.color}}></div>
                      
                      <div className="flex justify-between items-start mb-4 pl-3">
                          <h3 className="font-black text-xl text-slate-800 dark:text-white line-clamp-1">{queue.name}</h3>
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button onClick={() => handleOpenModal(queue)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400 hover:text-indigo-600"><Edit2 size={16} /></button>
                              <button onClick={() => handleDelete(queue.id)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg text-slate-400 hover:text-red-500"><Trash2 size={16} /></button>
                          </div>
                      </div>
                      
                      <div className="pl-3 space-y-3 mb-4 flex-1">
                          <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider">
                              <Palette size={14} /> Cor: <span style={{color: queue.color}}>{queue.color}</span>
                          </div>
                          {queue.greetingMessage && (
                              <div className="bg-slate-50 dark:bg-slate-900 p-3 rounded-xl">
                                  <p className="text-xs text-slate-600 dark:text-slate-300 italic line-clamp-2">"{queue.greetingMessage}"</p>
                              </div>
                          )}
                      </div>

                      <div className="pl-3 pt-3 border-t border-slate-100 dark:border-slate-700">
                          <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Atendentes Vinculados</p>
                          <div className="flex -space-x-2 overflow-hidden">
                              {queue.users && queue.users.length > 0 ? (
                                  queue.users.slice(0, 5).map((u) => (
                                      <div key={u.id} className="w-8 h-8 rounded-full border-2 border-white dark:border-slate-800 bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-300" title={u.name}>
                                          {u.name.charAt(0)}
                                      </div>
                                  ))
                              ) : (
                                  <span className="text-xs text-slate-400 italic">Nenhum atendente</span>
                              )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                ...
                <form onSubmit={handleSave} className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-5">
                    <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Nome do Setor</label>
                        <input 
                            type="text" 
                            ...
                            onChange={e => setCurrentQueue({...currentQueue, name: e.target.value})}
                            required
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Cor</label>
                        <div className="flex items-center gap-3">
                            <input 
                                type="color"
                                className="w-12 h-12 rounded-xl border-none cursor-pointer bg-transparent"
                                value={currentQueue.color}
                                onChange={e => setCurrentQueue({...currentQueue, color: e.target.value})}
                            />
                            <input 
                                type="text"
                                className="flex-1 p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl font-bold text-sm outline-none focus:border-indigo-500 uppercase"
                                value={currentQueue.color}
                                onChange={e => setCurrentQueue({...currentQueue, color: e.target.value})}
                            />
                        </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Mensagem de Saudação</label>
                        <textarea 
                            rows={3}
                            ...
                            onChange={e => setCurrentQueue({...currentQueue, greetingMessage: e.target.value})}
                            placeholder="Mensagem enviada quando o cliente entra nesta fila..."
                        />
                    </div>

                    <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-700">
                        <label className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Vincular Atendentes</label>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto custom-scrollbar p-1">
                            {availableUsers.map(user => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${isSelected ? 'bg-indigo-500 text-white' : 'bg-slate-200 dark:bg-slate-800 text-slate-500'}`}>
                                            {user.name.charAt(0)}
                                        </div>
                                        <span className={`text-sm font-bold truncate flex-1 ${isSelected ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-400'}`}>
                                            {user.name}
                                        </span>
