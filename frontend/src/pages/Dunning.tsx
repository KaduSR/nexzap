...
const Dunning: React.FC = () => {
  ...
  const [currentRule, setCurrentRule] = useState<Partial<DunningRule>>({
    ...
    days: 3,
    messageTemplate:
      "Olá {{name}}, sua fatura vence em breve. Segue o Pix: {{pix}}",
    isActive: true,
  });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = currentRule.id
        ? `${API_URL}/api/dunning/${currentRule.id}`
        : `${API_URL}/api/dunning`;
  ...
  const handleRunNow = async () => {
    if (
      !confirm(
        "Deseja forçar a execução da régua agora? Isso pode enviar mensagens reais."
      )
    )
      return;
    try {
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Automatize o envio de boletos e lembretes para reduzir a
            inadimplência.
          </p>
        </div>
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 font-bold text-xs uppercase tracking-widest transition-all"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          rules.map((rule) => (
            <div
              key={rule.id}
              className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm relative group overflow-hidden"
            >
              <div
                ...
                <div className="flex items-center gap-3">
                  <div
                    className={`p-3 rounded-xl ${
                      rule.frequencyType === "before_due"
                        ? "bg-blue-100 text-blue-600 dark:bg-blue-900/20"
                        : rule.frequencyType === "on_due"
            ...
            <form onSubmit={handleSave} className="p-8 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-slate-500 pl-1">
                  Variáveis disponíveis: <b>{"{{name}}"}</b>,{" "}
                  <b>{"{{valor}}"}</b>, <b>{"{{vencimento}}"}</b>,{" "}
                  <b>{"{{pix}}"}</b>, <b>{"{{linha_digitavel}}"}</b>,{" "}
                  <b>{"{{link_pdf}}"}</b>.
                </p>
              </div>

              <div
                className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-700 cursor-pointer"
                onClick={() =>
                  setCurrentRule({
                    ...currentRule,
                ...
                >
                  <div
                    className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${
                      currentRule.isActive ? "left-7" : "left-1"
                    }`}
                  ></div>
                </div>
