...
import { Play, Pause, Loader2, Captions, AlertTriangle, FileAudio } from 'lucide-react';
...
const AudioPlayer: React.FC<AudioPlayerProps> = ({ src, autoTranscribe = false }) => {
  ...
  const handleTranscription = useCallback(async () => {
    ...
    try {
      ...
      const transcriptionResponse = await fetch(`${API_URL}/api/ai/transcribe`, {
          method: 'POST',
          body: formData,
      });
    ...
    <div className="w-full min-w-[240px] max-w-md space-y-2">
        <div className="flex items-center gap-3 bg-white dark:bg-slate-900/80 p-2 pr-4 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm backdrop-blur-sm">
          <button
            ...
            disabled={isLoading}
            className={`w-10 h-10 flex items-center justify-center rounded-full text-white transition-all shrink-0 shadow-md ${isLoading ? 'bg-slate-400' : 'bg-indigo-600 hover:bg-indigo-700'}`}
          >
        ...
        <div className="flex justify-between items-start px-2">
             <button 
              ...
              >
                  {isTranscribing ? <Loader2 size={10} className="animate-spin" /> : <Captions size={10} />}
        ...
        {(transcriptionError || transcription) && (
             <div className={`p-3 text-xs rounded-xl border animate-in fade-in slide-in-from-top-2 ${transcriptionError ? 'bg-red-50 text-red-600 border-red-100' : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700'}`}>
                {transcriptionError ? transcriptionError : <p className="italic">"{transcription}"</p>}
            </div>
        )}
    </div>
  );
