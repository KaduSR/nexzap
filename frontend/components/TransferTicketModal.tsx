...
import { X, Search, User, GitMerge, Check, Loader2, ArrowRight } from 'lucide-react';
...
const TransferTicketModal: React.FC<Props> = ({ ticketId, onClose, onSuccess }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
        const queueRes = await fetch(`${API_URL}/api/queues`, { headers: { 'Authorization': 'Bearer token' } });
  ...
  const handleTransfer = async () => {
      ...
      setIsSaving(true);
      try {
          const payload = tab === 'queue' ? { queueId: selectedId, userId: null, status: 'pending' } : { userId: selectedId, queueId: null };
  ...
  const filteredList = tab === 'queue' 
    ? queues.filter(q => q.name.toLowerCase().includes(searchTerm.toLowerCase()))
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[32px] shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800 animate-in zoom-in-95 duration-200 flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 flex gap-2 border-b border-slate-100 dark:border-slate-800">
            <button 
                ...
                className={`flex-1 py-3 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${tab === 'queue' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}
            >
                <GitMerge size={16} /> Para Setor
            </button>
            <button 
                onClick={() => { setTab('user'); setSelectedId(null); }}
                className={`flex-1 py-3 rounded-xl font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${tab === 'user' ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}
            >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2 bg-slate-50 dark:bg-slate-950">
            {loading ? (
                ...
                <div className="text-center py-8 text-slate-400 text-xs font-bold uppercase">Nenhum resultado encontrado.</div>
            ) : (
                filteredList.map((item: any) => (
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white ${tab === 'queue' ? 'bg-indigo-500' : 'bg-emerald-500'}`} style={item.color ? {backgroundColor: item.color} : {}}>
                                {item.name.charAt(0)}
                            </div>
                            <span className="font-bold text-sm text-slate-700 dark:text-slate-200">{item.name}</span>
                        </div>
                        {selectedId === item.id && <Check className="text-indigo-600 dark:text-indigo-400" size={20} />}
                    </div>
                ))
            )}
        </div>
