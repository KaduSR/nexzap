...
const TicketAiPanel: React.FC<TicketAiPanelProps> = ({ ticketId, onClose }) => {
  ...
  const handleAnalyze = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/tickets/${ticketId}/ai-analysis`, {
          headers: { 'Authorization': 'Bearer token' }
      });
      if (res.ok) {
          setData(await res.json());
      } else {
          alert("Não foi possível gerar a análise. Verifique se há mensagens suficientes.");
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const getSentimentInfo = () => {
      if (!data) return { color: 'text-slate-400', bg: 'bg-slate-200', icon: <Meh /> };
      
      if (data.sentiment === 'negative') return { color: 'text-red-500', bg: 'bg-red-500', icon: <Frown size={28} /> };
      if (data.sentiment === 'positive') return { color: 'text-emerald-500', bg: 'bg-emerald-500', icon: <Smile size={28} /> };
      return { color: 'text-amber-500', bg: 'bg-amber-500', icon: <Meh size={28} /> };
  };
            ...
            {data && !loading && (
                <>
                    ...
                    <div className="space-y-2">
                        <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Humor do Cliente</h4>
                        <div className="flex items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
                            ...
                            <div className="flex-1 space-y-1">
                                <div className="flex justify-between text-xs font-bold">
                                    <span className="capitalize text-slate-700 dark:text-slate-200">{data.sentiment === 'positive' ? 'Satisfeito' : data.sentiment === 'negative' ? 'Insatisfeito' : 'Neutro'}</span>
                                    <span className={sentiment.color}>{data.sentimentScore}%</span>
                                </div>
                    ...
                    {/* Summary Section */}
                    <div className="space-y-2">
                        <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Resumo Executivo</h4>
                    ...
                    {/* Suggestion Section */}
                    <div className="space-y-2">
                        <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest">Sugestão de Resposta</h4>
